...
2. `make patch-plan ENV=stage` — заморозить набор pending SECURITY-обновлений
   в манифест `пакет=версия` (ничего не устанавливает). Между патчингом
   стейджа сегодня и раскаткой на prod завтра обычный `apt/dnf upgrade`
   может подтянуть уже ДРУГОЙ набор — на зеркало за это время прилетают
   новые апдейты. Манифест — это то, что делает утверждение «на prod едет
   ровно то, что проверено на стейдже» фактом, а не пожеланием.
3. `make patch ENV=stage` — патчинг ОС/middleware через Ansible. Если для
   среды есть замороженный манифест — ставит ровно его версии; если нет —
   как раньше, «что отдаёт security-репозиторий сейчас» (с явным notice об
   этом).
