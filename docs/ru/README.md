...
2. `make patch-plan ENV=prod` — заморозить набор pending SECURITY-обновлений
   ...
   Каждый пакет ещё привязывается к advisory, закрывшему его точную версию:
   ELSA на Oracle (из `dnf updateinfo`, лежит в метаданных репо) и USN на
   Ubuntu (в apt метаданных advisory нет, поэтому план обогащается из живого
   фида Ubuntu Security Notices от Canonical) — и то, и другое видно в HTML-отчёте.
...
Дельта сканов выполняет роль реестра находок: `make scan-delta` пишет
`results/delta-<env>.csv` — по строке на находку со слоем, владельцем,
severity, статусом (open/closed), датой обнаружения и колонками SLA,
готовыми к триажу.
